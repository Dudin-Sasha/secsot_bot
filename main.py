...
async def distribution(user_data, res):
    for admin_id in admins:
        ...
        try:
            ...
            if f_type == "photo":
                await bot.send_photo(chat_id=admin_id['TG_id'], photo=f_id, caption=res,parse_mode="Markdown")
            elif f_type == "document":
                await bot.send_document(chat_id=admin_id['TG_id'], document=f_id, caption=res,parse_mode="Markdown")
            elif f_type == "video":
                await bot.send_video(chat_id=admin_id['TG_id'], video=f_id, caption=res,parse_mode="Markdown")
            else:
                await bot.send_message(chat_id=admin_id['TG_id'], text=res, parse_mode="Markdown")
...
async def cmd_start(message: types.Message):
    await message.answer(__INFO["START_TEXT"], reply_markup=get_start_keyboard())


@router.message(F.text == "О Нас")
async def about_us(message: types.Message):
    await message.answer(__INFO['ABOUT_US_TEXT'])
...
async def site(message: types.Message):
    await message.answer(text=__INFO["LINK_TEXT"],parse_mode="Markdown")
...
async def cmd_start_anketa(message: types.Message, state: FSMContext):
    ...
    await message.answer(__INFO['DESCRIPTION_TEXT'], reply_markup=ReplyKeyboardRemove())    
...
async def procces_description(message: types.Message, state: FSMContext):
    ...
    if message.text:
        ...
        await message.answer(__INFO['TIME_TEXT'], parse_mode="Markdown")
    else:
...
async def process_time(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(__INFO['DATE_TEXT'], parse_mode="Markdown")
    except ValueError:
...
async def process_date(message: types.Message, state: FSMContext):
    ...
    if dateCheck(message.text):
        ...
        await message.answer(__INFO['FILE_TEXT'], 
...
async def skip_file_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(__INFO['CARD_NUMBER_TEXT'])
...
async def proccess_file(message: types.Message, state: FSMContext):
    ...
    await state.set_state(Form.cardNumber)
    await message.answer(__INFO['CARD_NUMBER_TEXT'])
...
async def card_handler(message: types.Message, state: FSMContext):
    ...
    if len(admins) >0:
        try:
            path = await bot.get_file(file_id=user_data.get('file'))
        except:
            path = None
...
async def register_admin(message: types.Message, command: CommandObject):
    global admins
    print(f"\n\n{command.args}\n\n")
    tmp = reg_new_admin(cmdArg=command.args, TG_id = message.from_user.id)
